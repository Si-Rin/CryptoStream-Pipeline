...
# Lire le Silver Layer
print("📖 Lecture du Silver Layer...")
df = spark.read.format("delta").load("C:/tmp/data/silver/trades")
print(f"Lignes dans Silver : {df.count()}")

# Fenêtre glissante par symbole ordonnée par temps
...
# Vérification
df_check = spark.read.format("delta").load("C:/tmp/data/gold/trades")
print(f"✅ Lignes dans Gold : {df_check.count()}")
df_check.select(
    "symbol", "window_start", "close", "rsi", "macd", "bb_upper", "bb_lower"
).show(10)